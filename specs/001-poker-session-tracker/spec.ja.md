...
- **FR-042**: システムはポット額と勝率（0-100%）を持つ個別のオールイン状況の記録を許可しなければならない
- **FR-043**: システムはAll-in EVをセッション内のすべてのオールインの（ポット額×勝率）の合計として計算しなければならない
- **FR-044**: システムはオールインサマリーを表示しなければならない: 総数、総ポット額、平均勝率、計算されたAll-in EV
...
- **AllInRecord（オールイン記録）**: 個別のオールイン状況；ポット額、勝率を持つ；セッションに属する；All-in EV計算に使用
