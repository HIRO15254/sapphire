...
本格的なプレイヤーとして、プレイ中にリアルタイムでセッションを記録したい。セッション中のスタック推移を追跡し、オールインのEV計算もリアルタイムで行えるようにするため。

**優先度の理由**: リアルタイム記録により、詳細なスタック追跡とEV分析が可能になる。上達に真剣なプレイヤーにとって価値があるが、アーカイブ記録より手間がかかる。

**独立テスト**: アクティブセッションの開始、スタック変動の記録、セッションの完了で完全にテスト可能。詳細なセッションタイムラインと収支グラフを提供することで価値を提供。

**受け入れシナリオ**:

1. **前提** ログイン済み、**操作** 店舗とゲームを指定してアクティブセッションを開始、**結果** 開始時刻付きでアクティブセッションが作成され、サイドバーにライブセッションへのリンクが表示される
2. **前提** アクティブセッション中、**操作** スタック額を更新、**結果** 現在時刻でスタック履歴が更新される
3. **前提** アクティブセッション中、**操作** リバイまたはアドオンを記録（時刻指定可能）、**結果** 指定時刻（デフォルトは現在時刻）と金額とともにログに記録される。時刻は最後のイベント以降のみ指定可能
4. **前提** アクティブセッション中、**操作** オールインを記録（時刻指定可能）、**結果** ポット額、勝率、結果が記録され、All-in EVがリアルタイムで更新される
5. **前提** アクティブセッション中、**操作** セッションを一時停止、**結果** 経過時間のカウントが停止し、一時停止状態が表示される
6. **前提** 一時停止中、**操作** セッションを再開、**結果** 経過時間のカウントが再開される（一時停止時間は除外）
7. **前提** アクティブセッション中、**操作** セッションを終了（時刻指定可能）、**結果** 指定時刻とキャッシュアウトでセッションが確定され、セッション履歴に追加される
8. **前提** ライブ記録したセッションを保有、**操作** セッション詳細を表示、**結果** イベント履歴（折りたたみ可能）と収支推移グラフ（サマリーと切り替え可能）が表示される。セッション時間は一時停止時間を除外した実プレイ時間が表示される
9. **前提** アクティブセッション中、**操作** ハンド数カウンタの+ボタンを押す、**結果** ハンド数が1増加し、hand_completeイベントが記録される（タイムラインには表示されない）
10. **前提** アクティブセッション中でハンド数が1以上、**操作** ハンド数カウンタの-ボタンを押す、**結果** 最新のhand_completeイベントが削除され、ハンド数が1減少する
11. **前提** 収支推移グラフを表示中、**操作** 横軸をハンド数に切り替え、**結果** グラフの横軸が経過時間からハンド数に変更される
...
4. **前提** アクティブセッション中、**操作** 空席をクリック、**結果** 仮プレイヤー（"Seat X"）が作成され、同卓者として追加される
5. **前提** 同卓者を追加済み、**操作** 同卓者をクリックして編集画面を開く、**結果** プレイヤー名（仮プレイヤーのみ）、タグ、メモを編集できる
6. **前提** 仮プレイヤーの同卓者を追加済み、**操作** 既存プレイヤーを割り当て、**結果** 仮プレイヤーのタグ・メモが既存プレイヤーにマージされ、同卓者が既存プレイヤーに紐付けられる
7. **前提** 仮プレイヤーの同卓者を追加済み、**操作** 新規プレイヤーとして作成、**結果** 仮プレイヤーが永続プレイヤーに変換される（タグ・メモは保持）
8. **前提** 永続プレイヤーに紐付けられた同卓者がいる、**操作** 同卓者一覧を表示、**結果** 永続プレイヤーの行には視覚的な区別（左ボーダー）が表示される
9. **前提** タグ付きの同卓者がいる、**操作** 同卓者一覧を表示、**結果** プレイヤーのタグが行内に表示される
10. **前提** メモ付きの同卓者がいる、**操作** 同卓者一覧を表示、**結果** プレイヤーの全般メモが行の下に表示される
11. **前提** セッション履歴のあるプレイヤーを保有、**操作** プレイヤーを表示、**結果** そのプレイヤーが参加したすべてのセッションが表示される
...
**セッション管理 - アクティブ**
- **FR-050**: システムは店舗とゲームを指定してアクティブセッションの開始を許可しなければならない。アクティブセッションが存在する場合、サイドバーにナビゲーションリンクを表示する
- **FR-051**: システムはアクティブセッション中のテーブルにいるプレイヤー（同卓者）を追跡しなければならない
- **FR-051a**: 同卓者は空席クリックで追加され、仮プレイヤー（"Seat X"）として作成される
- **FR-051b**: 同卓者をクリックするとプレイヤー編集画面が開き、名前（仮プレイヤーのみ）、タグ、全般メモを編集できる
- **FR-051c**: 仮プレイヤーは既存プレイヤーへの割り当て（タグ・メモのマージ）または新規プレイヤーとしての永続化が可能
- **FR-051d**: 同卓者一覧では、永続プレイヤーには視覚的な区別（左ボーダー）が表示され、タグと全般メモが表示される
- **FR-052**: システムはアクティブセッション中のハンド記録を許可しなければならない
- **FR-053**: システムはアクティブセッション中のスタック推移を追跡しなければならない。スタック更新は常に現在時刻で記録される
- **FR-054**: システムはタイムスタンプ付きでリバイとアドオンの記録を許可しなければならない。時刻は指定可能（デフォルトは現在時刻）で、最後のイベント以降の時刻のみ有効
- **FR-054a**: システムはアクティブセッション中のオールイン記録を許可しなければならない。時刻指定可能で、All-in EVがリアルタイムで計算される
- **FR-055**: システムはアクティブセッションを完了済みアーカイブセッションに変換できなければならない。キャッシュアウト時刻は指定可能
- **FR-056**: システムはアクティブセッションの一時停止と再開を許可しなければならない。経過時間は一時停止時間を除外して計算される
- **FR-057**: システムはライブ記録されたセッションの詳細ページでイベント履歴（折りたたみ可能）と収支推移グラフ（サマリーと切り替え可能）を表示しなければならない
- **FR-058**: 収支推移グラフは収支（緑）とAll-in調整収支（オレンジ）を表示し、スタック更新イベントごとにプロットする
- **FR-059**: システムはアクティブセッション中のハンド数カウンタを提供しなければならない。+ボタンで「hand_complete」イベントを追加し、-ボタンで最新の「hand_complete」イベントを削除する
- **FR-060a**: 収支推移グラフの横軸は経過時間またはハンド数を選択可能でなければならない
...
- **Session（セッション）**: プレイセッション（アーカイブまたはアクティブ）；店舗リンク、ゲームリンク、時間、バイイン、キャッシュアウト、メモを持つ；ハンド、オールイン記録、セッションイベントを含む
- **SessionEvent（セッションイベント）**: アクティブセッションのイベントログ；イベントタイプ（session_start/end/pause/resume、stack_update、rebuy、addon）、タイムスタンプ、イベントデータを持つ；セッションに属する；収支推移グラフとイベント履歴の表示に使用
