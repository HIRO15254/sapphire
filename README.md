...
- **ポーカーセッショントラッカー** - セッション記録・分析機能
  - ✅ セッション記録機能（日時、場所、バイイン、キャッシュアウト、プレイ時間）
  - ✅ セッション履歴表示（日付順ソート、フィルタリング）
  - ✅ 統計・分析機能（合計収支、セッション数、平均収支、場所別統計）
  - ✅ タグ機能（セッションへの複数タグ付け）
  - ✅ リッチテキストメモ（TipTapエディタ）

- **認証機能**
  - ✅ Google OAuth認証
  - ✅ GitHub OAuth認証
  - ✅ メールアドレス/パスワード認証（Credentials）
  - ✅ アカウントリンク（同一メールで複数認証方法を使用可能）
...
Sapphireでは3種類の認証方法が利用可能です：

#### OAuth認証（Google/GitHub）

- ホームページの「Googleでログイン」または「GitHubでログイン」をクリック
- 外部サービスで認証後、自動的にSapphireにログイン

#### メールアドレス/パスワード認証

1. ホームページの「メールアドレスでログイン」または「アカウントを作成」をクリック
2. メールアドレスとパスワード（8文字以上）を入力
3. ログインまたは新規登録完了

**Note**: 同じメールアドレスでOAuthとパスワード認証の両方を使用できます（アカウントリンク機能）。
...
- **認証機能** (`specs/005-refactor-auth-testing/`)
  - ✅ Google OAuth
  - ✅ GitHub OAuth
  - ✅ メールアドレス/パスワード認証
  - ✅ アカウントリンク
...
詳細は各specsディレクトリの `spec.md` を参照してください。
