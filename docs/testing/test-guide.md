...
- **対象**: コンポーネント、統合ワークフロー、ユーザーインタラクション
- **モック**: Tauri APIの完全なモック化システム
- **ヘルパー**: カスタムレンダー、テストデータファクトリー

#### バックエンド（Rust）
- **フレームワーク**: Rustの標準テストフレームワーク + Tokio
- **対象**: 全データベース操作、Tauriコマンド、エラーハンドリング
- **場所**: `src-tauri/src/lib.rs` 内の `#[cfg(test)]` モジュール
- **データベース**: テスト用一時SQLiteデータベース

### 2. 統合テスト（E2E）
- **フレームワーク**: Playwright
- **対象**: アプリケーション全体のユーザーシナリオ
- **環境**: 実際のTauriアプリケーション
- **ブラウザ**: Chromium、Firefox、WebKit

### 3. テストカバレッジ
- **ツール**: Vitest Coverage (V8)
- **レポート**: HTML、JSON、テキスト形式
- **閾値**: 70% (branches, functions, lines, statements)
...
# テストカバレッジ付きフロントエンドテスト
bun run test:coverage

# Rustユニットテスト（シングルスレッド）
bun run test:rust
